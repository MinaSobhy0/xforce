<x-filament-widgets::widget>
    ...
    @php
        $checkoutAppointments = $this->getReadyForCheckoutAppointments();
    @endphp
        ...
        <x-filament::section class="mt-6">
            <x-slot name="heading">
                <div class="flex items-center gap-2">
                    ...
                    {{ __('booking::reception.flow.ready_for_checkout') }}
                    <span class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold rounded-full bg-success-500 text-white">
                        {{ $checkoutAppointments->count() }}
                    </span>
                </div>
            </x-slot>

            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                @foreach($checkoutAppointments as $appointment)
                    <div class="bg-white dark:bg-gray-800 rounded-xl border-2 border-success-200 dark:border-success-700 shadow-sm hover:shadow-md transition-all overflow-hidden">
                        {{-- Header --}}
                        <div class="flex items-center justify-between px-3 py-2 bg-success-50 dark:bg-success-900/30 border-b border-success-200 dark:border-success-800">
                            ...
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full bg-success-100 text-success-700 dark:bg-success-800 dark:text-success-300">
                                <x-heroicon-o-check-circle class="w-3 h-3" />
                                {{ __('booking::appointments.statuses.completed') }}
                            </span>
                        </div>
                        ...
                        <div class="p-3 space-y-2">
                            {{-- Patient Name --}}
                            <div class="flex items-center gap-2">
                                <div class="w-10 h-10 rounded-full bg-success-100 dark:bg-success-900/50 flex items-center justify-center flex-shrink-0">
                                    <span class="text-sm font-bold text-success-700 dark:text-success-300">
                                        {{ strtoupper(substr($appointment->patient?->first_name ?? 'P', 0, 1)) }}
                                    </span>
                                </div>
                            ...
                            @if($appointment->service)
                                <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                                    ...
                                    <span class="truncate">{{ $appointment->service->translatable_name ?? $appointment->service->name }}</span>
                                </div>
                            @endif

                            {{-- Invoice Info --}}
                            @if($appointment->invoice)
                                <div class="flex items-center justify-between p-2 mt-2 rounded-lg bg-gray-50 dark:bg-gray-900/50">
                                    <div class="text-xs">
                                        <span class="text-gray-500 dark:text-gray-400">{{ __('billing::billing.fields.total') }}:</span>
                                        <span class="font-bold text-gray-900 dark:text-white">
                                            {{ number_format($appointment->invoice->total_minor / 100, 2) }}
                                        </span>
                                    </div>
                                    @if($appointment->invoice->paid_minor > 0)
                                        <div class="text-xs">
                                            <span class="text-gray-500 dark:text-gray-400">{{ __('billing::billing.fields.paid') }}:</span>
                                            <span class="font-medium text-success-600 dark:text-success-400">
                                                {{ number_format($appointment->invoice->paid_minor / 100, 2) }}
                                            </span>
                                        </div>
                                    @endif
                                </div>
