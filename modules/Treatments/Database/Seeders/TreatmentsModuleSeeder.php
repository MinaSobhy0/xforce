...
namespace Modules\Treatments\Database\Seeders;
...
class TreatmentsModuleSeeder extends Seeder
{
    public function run(): void
    {
        $this->command->info('Seeding Treatments module data...');

        $tenant = Tenant::where('slug', 'system')->first();

        if (!$tenant) {
            $this->command->warn('System tenant not found. Skipping treatment seeds.');
            return;
        }

        $this->seedConsentTemplates($tenant);
        $this->seedCategories($tenant);
        $this->seedTreatments($tenant);

        $this->command->info('Treatments module seeded successfully.');
    }
    ...
    private function seedTreatments(Tenant $tenant): void
    {
        $laserCategory = TreatmentCategory::where('tenant_id', $tenant->id)
            ->whereRaw("name->>'en' = ?", ['Laser Hair Removal'])
            ->first();

        $skinCategory = TreatmentCategory::where('tenant_id', $tenant->id)
            ->whereRaw("name->>'en' = ?", ['Skin Rejuvenation'])
            ->first();

        $injectablesCategory = TreatmentCategory::where('tenant_id', $tenant->id)
        ...
        foreach ($treatments as $treatmentData) {
            $treatmentData['tenant_id'] = $tenant->id;
            $treatmentData['is_active'] = true;

            Treatment::firstOrCreate(
                [
                    'tenant_id' => $tenant->id,
                    'name->en' => $treatmentData['name']['en'],
                ],
                $treatmentData
            );

            $name = $treatmentData['name']['en'];
            $this->command->info("  - Treatment created: {$name}");
        }
    }
}
