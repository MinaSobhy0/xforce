...
use Modules\Treatments\Filament\Resources\ConsentTemplateResource\Pages;
...
class ConsentTemplateResource extends Resource
{
    ...
    {
        return __('treatments::treatments.navigation.consent_templates');
    }

    public static function getModelLabel(): string
    {
        return __('treatments::treatments.labels.consent_template');
    }

    public static function getPluralModelLabel(): string
    {
        return __('treatments::treatments.labels.consent_templates');
    }
                        ...
                        Forms\Components\Grid::make(2)
                            ->schema([
                                Forms\Components\TextInput::make('name.en')
                                    ->label(__('treatments::treatments.consent.name') . ' (English)')
                                    ->required()
                                    ->maxLength(150),

                                Forms\Components\TextInput::make('name.ar')
                                    ->label(__('treatments::treatments.consent.name') . ' (Arabic)')
                                    ->required()
                                    ->maxLength(150),
                            ]),
                        ...
                        Forms\Components\Grid::make(3)
                            ->schema([
                                Forms\Components\TextInput::make('version')
                                    ->label(__('treatments::treatments.consent.version'))
                                    ->default('1.0')
                                    ->required()
                                    ->maxLength(10),

                                Forms\Components\TextInput::make('valid_days')
                                    ->label(__('treatments::treatments.consent.valid_days'))
                                    ->numeric()
                                    ->nullable()
                                    ->helperText('Leave empty for no expiration'),

                                Forms\Components\Toggle::make('is_active')
                                    ->label(__('treatments::treatments.consent.is_active'))
                                    ->default(true),
                            ]),
                ...
                Forms\Components\Section::make('Consent Content')
                    ->schema([
                        Forms\Components\RichEditor::make('content.en')
                            ->label(__('treatments::treatments.consent.content') . ' (English)')
                            ->required()
                            ->toolbarButtons([
                                'bold',
                                'italic',
                        ...
                        Forms\Components\RichEditor::make('content.ar')
                            ->label(__('treatments::treatments.consent.content') . ' (Arabic)')
                            ->required()
                            ->toolbarButtons([
                                'bold',
                                'italic',
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('translated_name')
                    ->label(__('treatments::treatments.consent.name'))
                    ->searchable(['name'])
                    ->sortable()
                    ->wrap(),
