    ...
    >
            ...
            <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <div class="flex items-center justify-between">
                    <h3 class="text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('treatments::treatments.category_tree.tree_view') }}
                    </h3>
                    <span class="text-xs text-gray-500 dark:text-gray-400">
                        {{ __('treatments::treatments.category_tree.drag_hint') }}
                    </span>
                </div>
            </div>
            ...
            <div class="p-4">
                <ul x-ref="rootList" class="space-y-2">
                    @foreach ($this->categories as $category)
                        <x-treatments::category-tree-item :category="$category" />
                    @endforeach
                </ul>
                ...
                @if ($this->categories->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">
                            {{ __('treatments::treatments.category_tree.no_categories') }}
                        </h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ __('treatments::treatments.category_tree.create_first') }}
                        </p>
                    </div>
                @endif
            </div>
        ...
        <div class="flex items-center gap-6 text-sm text-gray-500 dark:text-gray-400">
            <div class="flex items-center gap-2">
                ...
                <span>{{ __('treatments::treatments.category_tree.active') }}</span>
            </div>
            <div class="flex items-center gap-2">
                <div class="w-3 h-3 rounded-full bg-gray-300 dark:bg-gray-600"></div>
                <span>{{ __('treatments::treatments.category_tree.inactive') }}</span>
            </div>
        </div>
    </div>
</x-filament-panels::page>
