...
>
    ...
    >
        ...
        @if (isset($category['services_count']) && $category['services_count'] > 0)
            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                {{ $category['services_count'] }} {{ __('services::services.labels.services') }}
            </span>
        @endif
        ...
        @if (!empty($category['children']) && count($category['children']) > 0)
            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400">
                {{ count($category['children']) }} {{ __('services::services.category_tree.subcategories') }}
            </span>
        @endif
        ...
        <button
            ...
            title="{{ $category['is_active'] ? __('services::services.category_tree.click_deactivate') : __('services::services.category_tree.click_activate') }}"
        >
        ...
        <div class="flex items-center gap-1">
            <a
                href="{{ \Modules\Services\Filament\Resources\ServiceCategoryResource::getUrl('edit', ['record' => $category['id']]) }}"
    ...
    @if (!empty($category['children']) && count($category['children']) > 0)
        ...
        >
            @foreach ($category['children'] as $child)
                <x-services::category-tree-item :category="$child" :depth="$depth + 1" />
            @endforeach
        </ul>
    @endif
</li>
